...
void mlx_set_window_name(mlx_win_list_t *win_ptr, char *name)
{
	IBOutlet NSWindow *yourWindow = ((MlxWin *)(win_ptr->winid))->win;
	NSString *str = [NSString stringWithCString:name encoding:NSASCIIStringEncoding];
	[yourWindow setTitle:str];
}

void mlx_set_window_icon(mlx_ptr_t *mlx, char *png_img)
{
	NSString *str = [NSString stringWithCString:png_img encoding:NSASCIIStringEncoding];
	NSImage *image = [[NSImage alloc]initWithContentsOfFile:str];
	[(NSApplication *)mlx->appid setApplicationIconImage:image];
}
